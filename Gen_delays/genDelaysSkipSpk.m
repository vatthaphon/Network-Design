function [conns,delays]=genDelaysSkipSpk(conns,spk_times,N_ex,T,tol_eq)
N=size(spk_times,2);
delays=NaN(N);
...
for j=N_ex:1:N
    for i=1:1:N
        if (conns(i,j)==1)
            % Excitatory input arrives and makes the postsyn. neuron
            % suddenly spike.
...
for j=1:1:(N_ex-1)
    for i=1:1:N
        if (conns(i,j)==1)
            % Postsyn. neuron is excitatory.
            if (i>=N_ex)
                if (cmp(spk_times(1,j),spk_times(1,i),tol_eq)<0)
                    x=rand;
                    while ((cmp(x, 1, tol_eq)==0) || (cmp(x, 0, tol_eq)==0))
                        x=rand;
                    end
                    delays(i,j)=x*T+(spk_times(1,i)-spk_times(1,j));
                elseif (cmp(spk_times(1,j),spk_times(1,i),tol_eq)>0)
                    x=rand;
                    while (cmp(x, 1, tol_eq)==0)
                        x=rand;
                    end
                    delays(i,j)=x*(T+spk_times(1,i)-spk_times(1,j));
                else
