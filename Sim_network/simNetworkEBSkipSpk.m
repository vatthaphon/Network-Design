function simNetworkEBSkipSpk(epsiMat, tauMat, initVoltages, tSpikes, tBegin, tEnd, dt, dataMode, T, gamma_i, I_i, tol_eq, osc_types)
...
while (cmp(t,tEnd,tol_eq)<=0)
    ...
    %% Send spikes
    if sum(isSpiking>0)        
       if (isSpiking(1,spk_Skip_osc)==1)
           spk_Skip_cnt=spk_Skip_cnt+1;
           if (spk_Skip_cnt~=spk_Skip_at_number)
    ...
    if sum(isSpiking>0)
        ...
        if (isSpiking(1,ref_Osc)==1)
            if isnan(ref_V(1,ref_Osc))
                ref_V=voltages;
                pert_V(pert_V_i+1,:)=ref_V;
                pert_V_i=pert_V_i+1;                
            else
                pert_V(pert_V_i+1,:)=voltages;
                pert_V_i=pert_V_i+1;
            end
        end        
    end
...
for i=1:1:n
    voltages=pert_V(i,:);
    d=sqrt(sum((voltages-ref_V).^2));
    abs_V_diff(i)=d;
end
plot(abs_V_diff,'-*b');
xlabel('Time');
ylabel('L2(V,V_ref)');
title('\sigma_{I}=0.1 and \sigma_{E}=0.1');
