function simNetwork(epsiMat, tauMat, initVoltages, tSpikes, tBegin, tEnd, dt, dataMode, T, gamma_i, I_i, tol_eq)
...
while (t<=tEnd)
    ...
    voltages=update(voltages,dt,gamma_i,I_i);
    
    %% Check if the neurons are spiking
    isSpiking=doesNeuronSpiking(voltages);
...
for i=1:1:noOscs
    ...
    plot(inhArrivalTime(i,:),i,'b+','MarkerSize',4);
    plot(exArrivalTime(i,:),i,'ro','MarkerSize',4);
end
ylim([0 noOscs+1]);
    ...
    for i=1:1:noOscs
        if (spkCounts(i,1)>=1)
            if (cmp(spkArrivalArrays(i,1),t,tol_eq)<=0)
                ...
                isExcitatory=sum(sum(syns>0));  % When the coming spikes are excitatory, isExcitatory>0
                isInhibitory=sum(sum(syns<0));  % When the coming spikes are inhibitory, isInhibitory>0
...
%% Check if the neurons are spiking
function val=doesNeuronSpiking(voltages)
global neuronModel

    if (neuronModel==0)
        val=doesMSSpiking(voltages);
    elseif (neuronModel==1)
        val=doesLIFSpiking(voltages);
    end    
end

function val=doesMSSpiking(voltages)
    v_max=1.0;
    
    val=(voltages>=v_max);
end

function val=doesLIFSpiking(voltages)
    v_max=1.0;
